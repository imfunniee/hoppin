...
import { useChannelMessage, useConnectionState, useReadChannelState } from "@onehop/react";
...
const Home: NextPage = () => {
	...
	useChannelMessage<{ message: string }>(process.env.CHANNEL_ID as string, "ROOM_MESSAGE", (data) => {
			...
			if (input.startsWith("/")) {
				switch (input.split(" ")[0].replace("/", "").toLocaleLowerCase()) {
					case "name":
						...
						break;
					case "color":
						const tempColor = input.split("/color ")[1].match(/^#(?:[0-9a-f]{3}){1,2}$/i);
	...
	useEffect(() => {
		if (connection === "connected" && state?.subscription === "available") setConnected(true);
			...
			<div>
				<div className={styles.header}>
					...
					<div data-src={connected ? "true" : "false"}>
						{connected ? <i className="ri-check-double-fill"></i> : <i className="ri-loader-5-line"></i>}
					</div>
				</div>
				<div className={styles.chat_container}>
					{messages
						...
						.map((e) => (
							<div key={e.time}>
								<b style={{ color: e.color || "#ffffff" }}>{e.user}</b> <span>{new Date(e.time).toLocaleTimeString()}</span> :{" "}
								{e.text}
							</div>
						))}
				</div>
				<div className={styles.input_container}>
					<div>
						<input
							...
							disabled={!connected}
						/>
						<label>use /name [abc] and /color [#fff] to set your name and color respectively</label>
					</div>
